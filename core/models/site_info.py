...
class SiteInfo(TimeInfo, SEOBasicAbstract):
    ...
    site_name = models.CharField(max_length=255, null=True, blank=True, verbose_name='Site Name')
    site_url = models.URLField(max_length=2000, null=True, blank=True, verbose_name='Site URL')
...
class HomeSlide(TimeInfo):
    image = models.ImageField(upload_to='product_imgs/', null=True, blank=True, verbose_name='Image')
    site = models.ForeignKey(SiteInfo, on_delete=models.CASCADE, verbose_name='Site')
...
@receiver(models.signals.pre_save, sender=HomeSlide)
def auto_delete_product_image_on_change(sender, instance, **kwargs):
    if not instance.pk:
        return False 
    
    try:
        old_image = HomeSlide.objects.get(pk=instance.pk).image 
    ...
    try:
        ...
        print(f'Exception delete p image on change p image: {str(e)}')


@receiver(models.signals.post_delete, sender=HomeSlide)
    ...
    if instance.image:
        if os.path.isfile(instance.image.path):
            try:
                ...
                print(f'Exception delete p image on delete p image: {str(e)}')
